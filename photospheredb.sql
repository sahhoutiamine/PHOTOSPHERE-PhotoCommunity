...
CREATE TABLE users (id INT AUTO_INCREMENT PRIMARY KEY,username VARCHAR(100) NOT NULL UNIQUE,email VARCHAR(255) NOT NULL UNIQUE,password VARCHAR(255) NOT NULL,createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,lastLogin DATETIME NULL,bio VARCHAR(600),profilePicture VARCHAR(600),uploadCount INT DEFAULT 0,subscriptionStart DATETIME NULL,subscriptionEnd DATETIME NULL,level VARCHAR(100),isSuper BOOLEAN DEFAULT FALSE);
...
CREATE TABLE photo_tags (photoId INT NOT NULL,tagId INT NOT NULL,createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY (photoId, tagId),FOREIGN KEY (photoId) REFERENCES photos(id) ON DELETE CASCADE,FOREIGN KEY (tagId) REFERENCES tags(id) ON DELETE CASCADE);
